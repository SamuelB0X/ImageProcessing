...
public class ImageProcessing {
    ...
    public static int[][] shrinkVertically(int[][] imageTwoD) {
        ...
        for (int i = 0; i < imageTwoD.length; i += 2) {
            for (int j = 0; j < imageTwoD[i].length; j++) {
                ...
                shrinkImage[i / 2][j] = imageTwoD[i][j];
            }
        }
        return shrinkImage;
    }
    ...
    public static int[][] paintRectangle(int[][] canvas, int width, int height, int iPosition, int jPosition, int color) {
        for (int i = iPosition; i < iPosition + height && i < canvas.length; i++) {
            for (int j = jPosition; j < jPosition + width && j < canvas[i].length; j++) {
                canvas[i][j] = color;
            }
        }
        return canvas;
